...
# → a table of shape (256, 768) for context_length=256
# → valid row indices: 0, 1, 2, ..., 255
```
...
If `num_tokens = 300`, it tries to look up row 256, 257, ..., 299. Those rows **do not exist** in the table. PyTorch raises an index out-of-bounds error and training crashes.

---

## A Concrete Example

Say `context_length = 256` and you pass a sequence of 300 tokens:
...
token 256 → pos_emb[256] ✗  row doesn't exist → CRASH
```
...
You might expect the model to simply ignore tokens beyond position 255. It doesn't. The model tries to look up a positional embedding for every token in the input. There is no automatic truncation — that must happen **explicitly at the dataset level** before data ever reaches the model.
...
With `max_tokens = context_length`, every sequence is trimmed to at most 256 tokens. The positional embedding table is never asked for an out-of-range row. The assert always passes.
